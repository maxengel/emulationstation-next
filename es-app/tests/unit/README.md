...
(#186 PL-08, what the game index owes a game) and `es-core/src/utils/TimeText.cpp`
(#195, the time of day beside a save's date: the 12-hour switch and a locale
with no AM/PM), tested with doctest (`external/doctest/doctest.h`); the binary
compiles those five files, `es-core/src/utils/StringUtil.cpp`, and the tests,
and no more.
